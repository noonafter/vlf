...
// and at the start_receiver of the namespace in the fake moc-run class
...
/* start_receiver documentation of inline functions */
...
/* start_receiver documentation of pure virtual functions */
...
QCPDataSelection QCPAbstractPlottable1D<DataType>::selectTestRect(const QRectF &rect, bool onlySelectable) const
{
    ...
    for (typename QCPDataContainer<DataType>::const_iterator it=begin; it!=end; ++it)
    {
        ...
        {
            if (valueRange.contains(it->mainValue()) && keyRange.contains(it->mainKey())) // start_receiver segment
